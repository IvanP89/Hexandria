//////////////////////////////////////////////////////////////////////////////
// Nombre:      Hexandria/ventana_libros.cpp
// Autor:       Iván Pizarro.
//
// Implementaciones de los métodos y funciones declarados en ventana_libros.h
//////////////////////////////////////////////////////////////////////////////
...
ventana_libros::ventana_libros ( wxWindow *_parent, Programa *_programa ): vLibros( _parent ) {
	...
	MyToolbar->EnableTool( id_vl_boton_borrar, false );
	MyToolbar->EnableTool( id_vl_boton_prestar, false );
	MyToolbar->EnableTool( id_vl_boton_devolver, false );
	MyToolbar->EnableTool( id_vl_boton_detalles, false );
	this->refresh();
...
// SELECCION
void ventana_libros::seleccion ( const bool &_seleccionado ) {
	
	if ( _seleccionado == true ) {
		
		grilla_libros->SelectRow( fila );
		
		MyToolbar->EnableTool( id_vl_boton_borrar, true );
		MyToolbar->EnableTool( id_vl_boton_prestar, true );
		MyToolbar->EnableTool( id_vl_boton_devolver, true );
		MyToolbar->EnableTool( id_vl_boton_detalles, true );
...
// EVENTOS GRILLA
void ventana_libros::refresh ( const bool &_conservar_seleccion ) {
	...
	if ( buscado != "" ) {
		...
		programa->libros.sort( orden_libros->GetSelection() );
		...
		for ( int i=0;i<tamanyo;i++ ) { 
			...
			grilla_libros->SetCellValue( i, 0, lib->get_titulo().c_str() );
			grilla_libros->SetCellValue( i, 1, lib->get_nombre_autores().c_str() );
			grilla_libros->SetCellValue( i, 2, lib->get_publicacion().c_str() );
			if ( lib->get_tipo_prestamo() == a_domicilio ) {
				
				grilla_libros->SetCellValue( i, 3, wxT("a domicilio") );
				
			} else {
				
				grilla_libros->SetCellValue( i, 3, wxT("consulta") );
				
			}
			grilla_libros->SetCellValue( i, 4, lib->get_ubicacion().str().c_str() );
...
void ventana_libros::click_izq_grilla ( wxGridEvent &_event ) {
...
void ventana_libros::Dclick_grilla ( wxGridEvent &_event ) {
	
	ventana_info_libro   nueva_ventana( this, pos_real[fila] );
	nueva_ventana.ShowModal();
	
}

void ventana_libros::click_der_grilla ( wxGridEvent &_event ) {
	...
	grilla_libros->PopupMenu( menu_grilla, posicion_click );
	
}

// BARRA DE HERRAMIENTAS
void ventana_libros::click_boton_borrar ( wxCommandEvent &_event ) {
	
	string aux;
	aux += "¿Seguro que desea eliminar \"";
	aux += programa->libros.get_libro_pos( pos_real[fila] ).get_titulo();
	aux += "\"?";
	
	if ( wxMessageBox( aux.c_str(), wxT("Eliminar libro..."), wxYES_NO ) == wxYES && fila >= 0 ) {
		
		if ( programa->libros.pop_pos( pos_real[fila] ) == false ) {
			...
			if ( fila == programa->libros.get_cant_libros() ) {
...
void ventana_libros::click_boton_agregar ( wxCommandEvent &_event ) {
	
	this->seleccion( false );
	
	ventana_agregar_libro   nueva_ventana( this, -1 );
...
void ventana_libros::click_editar_libro( wxCommandEvent& event )  {
	
	ventana_agregar_libro   nueva_ventana( this, pos_real[fila] );
...
void ventana_libros::click_boton_prestar ( wxCommandEvent &_event ) {
	
	libro *lib = &( programa->libros.get_libro_pos( pos_real[fila] ) );
	int cod_libro = lib->get_codigo();
	...
	if ( lib->get_prestado() == true ) {
		...
		wxMessageBox( msg.c_str(), wxT("Aviso") );
		
	} else {
		
		ventana_socios   nueva_ventana( this, programa );
		int nro_socio = nueva_ventana.ShowModal();
		
		lib = &( programa->libros.get_libro( cod_libro ) ); // Por si se realizaron cambios desde ventana_socios.
		
		if ( nro_socio >= 0 ) {
			
			socio *soc = &( programa->socios.get_socio_cod( nro_socio ) );
...
void ventana_libros::click_boton_devolver ( wxCommandEvent &_event ) {
	
	libro *lib = &( programa->libros.get_libro_pos( pos_real[fila] ) );
	socio *soc = &( programa->socios.get_socio_cod( lib->get_nro_socio_prestado() ) );
...
void ventana_libros::click_boton_detalles ( wxCommandEvent &_event ) {
	
	ventana_info_libro   nueva_ventana( this, pos_real[fila], programa );
	nueva_ventana.ShowModal();
	
}

void ventana_libros::click_boton_estadisticas ( wxCommandEvent &_event ) {
...
void ventana_libros::cambio_orden ( wxCommandEvent &_event ) {
...
void ventana_libros::click_buscar ( wxCommandEvent &_event ) {
	...
	if ( buscado == "" ) {
		...
		for ( int i=0;i<tamanyo;i++ ) { 
			...
			if ( lib->buscar_contenido( buscado ) == true ) {
				...
				grilla_libros->SetCellValue( contador, 0, lib->get_titulo().c_str() );
				grilla_libros->SetCellValue( contador, 1, lib->get_nombre_autores().c_str() );
				grilla_libros->SetCellValue( contador, 2, lib->get_publicacion().c_str() );
				if ( lib->get_tipo_prestamo() == a_domicilio ) {
					
					grilla_libros->SetCellValue( contador, 3, wxT("a domicilio") );
					
				} else {
					
					grilla_libros->SetCellValue( contador, 3, wxT("consulta") );
					
				}
				grilla_libros->SetCellValue( contador, 4, lib->get_ubicacion().str().c_str() );
...
void ventana_libros::click_boton_restaurar ( wxCommandEvent &_event ) {
...
void ventana_libros::click_boton_CDU ( wxCommandEvent &_event ) {
...
void ventana_libros::click_boton_ayuda ( wxCommandEvent &_event ) {
...
// EVENTOS CDU
void ventana_libros::principal_elegido ( wxCommandEvent &_event ) {
	...
	for ( int i=0;i<tamanyo;i++ ) { 
		...
		if ( lib->get_ubicacion().c1 == eleccion1 ) {
...
void ventana_libros::sub_1_elegido ( wxCommandEvent &_event ) {
	...
	for ( int i=0;i<tamanyo;i++ ) { 
		...
		if ( lib->get_ubicacion().c1 == eleccion1 && lib->get_ubicacion().c2 == eleccion2 ) {
...
void ventana_libros::sub_2_elegido ( wxCommandEvent &_event ) {
	...
	for ( int i=0;i<tamanyo;i++ ) { 
		...
		if ( lib->get_ubicacion().c1 == eleccion1 && lib->get_ubicacion().c2 == eleccion2 && lib->get_ubicacion().c3 == eleccion3 ) {
...
// BOTONES VENTANA
void ventana_libros::click_boton_aceptar ( wxCommandEvent &_event ) {
	
	if ( anterior != NULL ) { // Modo elegir un libro para realizar préstamo.
		
		if ( MyToolbar->GetToolEnabled( id_vl_boton_borrar ) == false ) {
			
			wxMessageBox( wxT("Debe seleccionar un libro para realizar el préstamo. Si ya no desea llevar a cabo esta acción, cierre la ventana con el botón \"cancelar\""), wxT("Aviso") );
			
		} else if ( programa->libros.get_libro_pos( pos_real[fila] ).get_prestado() == true ) {
			
			wxMessageBox( wxT("El libro seleccionado está prestado, por lo que no se encuentra disponible."), wxT("Aviso") );
			
		} else if ( cambios == true ) {
			
			anterior->set_cambios_sombra( programa );
			int cod_libro = programa->libros.get_libro_pos( pos_real[fila] ).get_codigo();
			EndModal( cod_libro );
...
void ventana_libros::click_boton_cancelar ( wxCommandEvent &_event ) {
