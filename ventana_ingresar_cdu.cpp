//////////////////////////////////////////////////////////////////////////////
// Nombre:      Hexandria/ventana_ingresar_cdu.cpp
// Autor:       Iván Pizarro.
//
// Implementaciones de los métodos y funciones declarados en
// ventana_ingresar_cdu.h
//////////////////////////////////////////////////////////////////////////////
...
ventana_ingresar_cdu::ventana_ingresar_cdu( wxWindow *_parent, ventana_agregar_libro *_anterior ): vIngresarCDU( _parent ) {
	
	anterior = _anterior;
	elec_1_hecha = elec_2_hecha = elec_3_hecha = false;
	
}

ventana_ingresar_cdu::~ventana_ingresar_cdu() {
	
}

void ventana_ingresar_cdu::principal_elegido( wxCommandEvent &_event )  {
...
void ventana_ingresar_cdu::sub_1_elegido( wxCommandEvent &_event )  {
...
void ventana_ingresar_cdu::sub_2_elegido( wxCommandEvent &_event )  {
	
	elec_3_hecha = true;
	
}

void ventana_ingresar_cdu::click_boton_aceptar( wxCommandEvent &_event )  {
	
	if ( elec_1_hecha == false ) {
		
		wxMessageBox( wxT("Debe elegir un grupo principal."), wxT("Campos sin completar.") );
		
	} else if ( elec_2_hecha == false ) {
		
		wxMessageBox( wxT("Debe elegir un subgrupo 1."), wxT("Campos sin completar.") );
		
	} else if ( elec_3_hecha == false ) {
		
		wxMessageBox( wxT("Debe elegir un subgrupo 2."), wxT("Campos sin completar.") );
		
	} else {
		
		CDU nueva_ubicacion;
		nueva_ubicacion.c1 = eleccion_principal->GetSelection();
		nueva_ubicacion.c2 = eleccion_sub_1->GetSelection();
		nueva_ubicacion.c3 = eleccion_sub_2->GetSelection();
		string aux = eleccion_inicial_ape->GetString( eleccion_inicial_ape->GetSelection() ).c_str();
		nueva_ubicacion.inicial_ape_autor = aux[0];
		aux = eleccion_inicial_nom->GetString( eleccion_inicial_nom->GetSelection() ).c_str();
		nueva_ubicacion.inicial_nom_autor = aux[0];
		
		anterior->nuevo.set_ubicacion( nueva_ubicacion );
		
		anterior->renglon_ubicacion->SetValue( nueva_ubicacion.str().c_str() );
...
void ventana_ingresar_cdu::click_boton_cancelar( wxCommandEvent &_event )  {
